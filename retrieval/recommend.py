...
def build_user_query_vector(
    ...
    If weighted=False:
        every usable rated movie gets weight 1.0.

    If positive and negative signals cancel, falls back to
    liked-only positive weights to avoid a zero query vector.
    """
    ...
    if weighted:
        rated["weight"] = rated["rating"].astype(np.float32) - float(rating_midpoint)
    else:
        rated["weight"] = 1.0
    ...
    if rated.empty:
        raise ValueError("No usable ratings found after centering around the neutral point.")
